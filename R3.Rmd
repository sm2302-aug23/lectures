...
We're most interested in the following packages:
...
- Observations: 1, 2, 3, 4, 5, 6
- Notice that
   - Amount of particles for each city is segregated into two groups: large and small.
...
```{r}
x <- storms[, 1:3]  # First 3 columns of storms
y <- storms$date  # The last column of storms
bind_cols(x, y)
```
...
```{r}
x <- storms[1:3, ]  # First 3 columns of storms
y <- storms[4:6, ]  # Last 3 columns of storms
bind_rows(x, y)
```
